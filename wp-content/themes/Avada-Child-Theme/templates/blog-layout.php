...
if(!is_search()) {
	...
	//$blog_cat_menu = wp_get_nav_menu_items('98');
	echo '<div class="mast_headline">' . $cat_title . '</div>';
	//dynamic_sidebar( 'avada-custom-sidebar-blogcategories' );
	//echo '</div>';
}
?>
		...
		<?php while ( have_posts() ) : ?>
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class( $post_classes ); ?>>
				...
				<div class="fusion-post-content post-content">
					<?php
					/* Get updated permalink for anchor links for Dev Docs and Support posts for Title link and Read more link */
					...
					/*$anchor_title = str_replace(home_url(), "", get_permalink(get_the_ID()));
					$anchor_title = substr($anchor_title, 1, strlen($anchor_title) - 2);*/
						
					// Render the post title.		
					if ($parent_cat == 5) { //5 dev
						echo '<h2 class="entry-title fusion-post-title"><a href="/developers#' . $post->post_name . '">' . get_the_title() . '</a></h2>';
					} else if ($parent_cat == 13) { //13 support
						echo '<h2 class="entry-title fusion-post-title"><a href="/support#' . $post->post_name . '">' . get_the_title() . '</a></h2>';
					} else {
						...
						<?php if ( 'grid' === $blog_layout || 'masonry' === $blog_layout || 'timeline' === $blog_layout ) : ?>
							...
							<div class="fusion-alignleft">
								<?php if ( Avada()->settings->get( 'post_meta_read' ) ) : ?>
									...
									if ($parent_cat == 5) { //5 dev
										echo '<a href="/developers#' . $post->post_name . '" class="fusion-read-more"' . $link_target . '>';
									} else if ($parent_cat == 13) { //13 support
										echo '<a href="/support#' . $post->post_name . '" class="fusion-read-more"' . $link_target . '>';
									} else {
										echo '<a href="' . esc_url_raw( get_permalink() ) . '" class="fusion-read-more"' . $link_target . '>';
									}
									?>
